...
pub enum WrCliError {
    ...
    UnknownSubcommand { name: String, parent: String },
    MissingRequiredFlag(String),
    InvalidFlagValue { flag: String, expected: &'static str, got: String },
    ArgValidationFailed(String),
    CommandHasNoRunner(String),

    // Config errors
    ConfigFileNotFound { name: String, paths: Vec<String> },
    ConfigParseError { path: String, source: String },
...
impl fmt::Display for WrCliError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            WrCliError::InvalidFlagValue { flag, expected, got } => {
                write!(
                    f,
            ...
            WrCliError::UnsupportedConfigFormat(ext) => {
                write!(
                    f,
                    "unsupported config format '{}' (supported: toml, json)",
                    ext
                )
            }
